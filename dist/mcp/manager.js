        ...
        this.proc.on('error', (err) => {
            ...
            const detail = err.code === 'ENOENT'
                ...
                : err.code === 'EINVAL'
                    ...
                    : err.message;
            const spawnError = new Error(`Failed to start MCP server "${config.name}": ${detail}`);
...
class McpManager {
     ...
     */
    async installAndStartServer(config) {
        ...
        const runningIdx = this.servers.findIndex((s) => s.serverName === config.name);
        if (runningIdx >= 0) {
    ...
    saveConfigs(configs) {
        ...
        fs.writeFileSync(MCP_CONFIG_FILE, JSON.stringify({ servers: configs }, null, 2), { mode: 0o600 });
    }
